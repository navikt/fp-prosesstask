...
interface TaskManagerFeil extends DeklarerteFeil {
    ...
    @TekniskFeil(feilkode = "PT-739415", feilmelding = "Transient datase connection feil, venter til neste runde (runde=%s): %s: %s", logLevel = LogLevel.WARN, exceptionClass = ProsessTaskMidlertidigException.class)
    Feil midlertidigUtilgjengeligDatabase(Integer round, Class<?> exceptionClass, String exceptionMessage);

    @TekniskFeil(feilkode = "PT-996896", feilmelding = "Kunne ikke polle database, venter til neste runde(runde=%s)", logLevel = LogLevel.WARN, exceptionClass = ProsessTaskException.class)
    ...
    @TekniskFeil(feilkode = "PT-530440", feilmelding = "Kunne ikke prosessere task pga transient database feil: id=%s, taskName=%s. Vil automatisk prøve igjen", logLevel = LogLevel.WARN, exceptionClass = ProsessTaskMidlertidigException.class)
    Feil kunneIkkeProsessereTaskTransientFeilVilPrøveIgjen(Long taskId, String taskName, Exception e);
    
    @TekniskFeil(feilkode = "PT-853672", feilmelding = "Kunne ikke prosessere task da blokkerende task kjører eller er allerede kjørt. Venter litt og forsøker igjen. id=%s, taskName=%s, status=%s, gruppe=%s. Vil automatisk prøve igjen", logLevel = LogLevel.WARN, exceptionClass = ProsessTaskMidlertidigException.class)
    Feil kunneIkkeProsessereTaskVetoForsøkerIgjen(Long taskId, String taskName, ProsessTaskStatus prosessTaskStatus, String gruppe);

    @TekniskFeil(feilkode = "PT-265358", feilmelding = "Kunne ikke lagre task, skriving til database mislykkes: task=%s, kjøresEtter=%s, parametere=%s.", logLevel = LogLevel.WARN, exceptionClass = ProsessTaskException.class)
    Feil feilVedLagringAvProsessTask(String taskName, LocalDateTime runAfter, Properties params, SQLException e);

    @TekniskFeil(feilkode = "PT-314160", feilmelding = "Finner ikke sql i fil=%s. Konfigurasjon er ugyldig.", logLevel = LogLevel.ERROR, exceptionClass = ProsessTaskKonfigException.class)
    Feil finnerIkkeSqlFil(String pollSqlFilNavn, IOException e);

    @TekniskFeil(feilkode = "PT-314162", feilmelding = "Pollet task for kjøring: id=%s, type=%s, originalException=%s", logLevel = LogLevel.WARN, exceptionClass = ProsessTaskException.class)
    ...
    @TekniskFeil(feilkode = "PT-900909", feilmelding = "Kan ikke kjøre task [%s, %s], fikk veto. Blokkert av task [%s]: %s", logLevel = LogLevel.INFO, exceptionClass = ProsessTaskException.class)
